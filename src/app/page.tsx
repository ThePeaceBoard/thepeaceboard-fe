...
export default function Home() {
  ...
  const toggleFullscreen = () => {
    ...
    console.log("Found control panels:", controlPanels.length);
  ...
  useEffect(() => {
    ...
    if (!peaceMapRef.current || !containerRef.current || !secondMapSection) {
      console.log("Missing refs:", {
        map: peaceMapRef.current,
        container: containerRef.current,
        secondMap: secondMapSection
      });
      return;
    }
    ...
    const handleScroll = (direction: "up" | "down") => {
      ...
      const sectionOffsets = sectionsRef.current.map((section, index) => {
        ...
        if (index === sectionsRef.current.length - 1) {
          return section.offsetTop - (container?.offsetTop ?? 0) + (footer?.offsetHeight ?? 0);
        }
        return section.offsetTop - (container?.offsetTop ?? 0);
      });

      const adjustedOffsets = [
        ...sectionOffsets,
        sectionOffsets[sectionOffsets.length - 1] + (sectionsRef.current.at(-1)?.offsetHeight ?? 0),
      ];
    ...
    <SonarProvider>
      <div ref={containerRef} className="app-container will-change-auto pointer-events-auto overflow-x-hidden"> 
        ...
        <div className="animated-page will-change-auto scene flex flex-col pointer-events-auto overflow-x-hidden">
          ...
          <section ref={firstSectionRef} className="first flip will-change-auto scene text-center max-w-full z-max">
            <div ref={heroRef}>
          ...
          <section className="second tracking-wider gap-14 w-full px-6">
            <div className="flex justify-start">
              <SocialBar/>
            </div>
            <h1 className="section-title flex flex-row gap-2 flex-wrap">
              <span>We are</span>
          ...
          <section className="third tracking-wider w-full px-6">
            <div className="flex flex-row justify-around gap-8 md:gap-16 lg:gap-32 flex-wrap">
              <div className="world-peace-paragraph flex flex-col gap-4">
                <p>
                  WE'VE BUILT A <span className="highlight">Peer-to-Peer PEACEBOARD</span> SHOWCASING
                ...
                <p>
                  ...
                  DEMAND FOR PEACE AND DEMONSTRATE THEIR COLLECTIVE POWER.
                </p>
              </div>
          ...
          <section className="fourth gap-16 tracking-wider font-bebas w-full px-6">
            <div className="flex flex-row justify-between gap-10 flex-wrap">
              <h1 className="section-title subscribe-title text-left">
                <span>AND WE'RE LAUNCHING </span>
                <span className="highlight"> SOON</span>
              </h1>
              <div className="align-middle">
                <MinimalCountdown targetDate={new Date("2025-02-30T00:00:00")}/>
              </div>
            </div>
            <div className="flex flex-row justify-start">
              <p className="section3-paragraph text-left">
